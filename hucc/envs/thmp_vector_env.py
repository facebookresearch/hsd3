...
#
# This is a clone of gym's AsyncVectorEnv
# (https://github.com/openai/gym/blob/master/gym/vector/async_vector_env.py),
# but using torch multiprocessing instead of the default Python one.
...
class TorchAsyncVectorEnv(VectorEnv):
    ...
    def __init__(
        ...
        if (observation_space is None) or (action_space is None):
            ...
            dummy_env.close()
            del dummy_env
        super(TorchAsyncVectorEnv, self).__init__(
        ...
        if self.shared_memory:
            try:
                ...
                raise ValueError(
                    'Using `shared_memory=True` in `TorchAsyncVectorEnv` '
    ...
    def _check_observation_spaces(self):
        ...
        for pipe in self.parent_pipes:
            pipe.send(
                ('_check_observation_space', self.single_observation_space)
            )
